...
const serviceConfig = {
  vol: {
    ...
    fields: [
      ...
      { name: 'message', label: 'Demandes particulières', type: 'textarea', required: false, col: 2, placeholder: 'Saisissez votre demande...' }
    ]
  },
  hotel: {
  ...
  vehicule: {
    ...
    fields: [
      ...
      { name: 'transmission', label: 'Transmission', type: 'select', required: true, col: 1, options: [{ value: 'manual', label: 'Manuelle' }, { value: 'automatic', label: 'Automatique' }] },
      { name: 'additional_driver', label: 'Conducteur supplémentaire', type: 'select', required: true, col: 1, options: [{ value: 'no', label: 'Non' }, { value: 'yes', label: 'Oui' }] },
      { name: 'insurance', label: 'Assurance', type: 'select', required: true, col: 1, options: [{ value: 'basic', label: 'Basique' }, { value: 'full', label: 'Tous risques' }] },
      { name: 'message', label: 'Demandes particulières', type: 'textarea', required: false, col: 2, placeholder: 'Saisissez votre demande...' }
    ]
  },
  excursion: {
    ...
    fields: [
      ...
      { name: 'duration', label: 'Durée', type: 'select', required: true, col: 1, options: [{ value: 'half_day', label: 'Demi-journée' }, { value: 'full_day', label: 'Journée complète' }, { value: 'multi_day', label: 'Plusieurs jours' }] },
      { name: 'participants', label: 'Nombre de participants', type: 'number', required: true, col: 1, min: 1, default: 1 },
      { name: 'tour_type', label: 'Type de circuit', type: 'select', required: true, col: 1, options: [{ value: 'private', label: 'Privé' }, { value: 'group', label: 'En groupe' }] },
      { name: 'language', label: 'Langue du guide', type: 'select', required: true, col: 1, options: [{ value: 'fr', label: 'Français' }, { value: 'en', label: 'Anglais' }, { value: 'm', label: 'Mooré' }, { value: 'd', label: 'Dioula' }] },
      { name: 'difficulty', label: 'Niveau de difficulté', type: 'select', required: false, col: 1, options: [{ value: 'easy', label: 'Facile' }, { value: 'moderate', label: 'Modéré' }, { value: 'hard', label: 'Difficile' }] },
      { name: 'message', label: 'Demandes particulières', type: 'textarea', required: false, col: 2, placeholder: 'Saisissez votre demande...' }
    ]
  }
};
...
const buildWhatsAppUrl = (data, phoneNumber) => {
  ...
  const dureeLabels = { half_day: 'Demi-journée', full_day: 'Journée complète', multi_day: 'Plusieurs jours' };
  const langueLabels = { fr: 'Français', en: 'Anglais', m: 'Mooré', d: 'Dioula' };
  const tourTypeLabels = { private: 'Privé', group: 'En groupe' };
  const difficultyLabels = { easy: 'Facile', moderate: 'Modéré', hard: 'Difficile' };
...
export default function DynamicReservation() {
  ...
  useEffect(() => {
    if (config?.fields) {
      const defaults = {};
      config.fields.forEach(field => { if (field.default !== undefined) defaults[field.name] = field.default; });
  ...
  const handleSubmit = async (e) => {
    ...
    setWaUrl(null);
    try {
      const API_URL = `${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/reservations`;
      const payload = { ...formData, service_type: type };
      const cleanPayload = Object.fromEntries(Object.entries(payload).filter(([_, v]) => v !== '' && v !== null && v !== undefined));
      ...
      if (!response.ok) {
        if (response.status === 422) {
          ...
          const errors = Object.entries(errorData.errors || {}).map(([f, m]) => `${f}: ${m.join(', ')}`).join(' | ');
      ...
      const waLink = buildWhatsAppUrl({ ...cleanPayload, service_type: type }, '22674199797');
      setWaUrl(waLink);
    ...
    return (
      <div className="min-h-screen bg-blue-100 py-12 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <Link to="/reservation" className="text-blue-600 hover:underline"> Retour aux services</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-blue-100 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-6 md:mb-8">
          <span className="text-4xl md:text-6xl block mb-3">{config.icon}</span>
          <h1 className="text-2xl md:text-4xl font-bold text-red-800">{config.title}</h1>
        </div>
        ...
        <div className="bg-white rounded-2xl border-2 border-red-700 p-4 md:p-8 lg:p-10">
          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                ...
                <div key={field.name} className={field.col === 2 ? 'md:col-span-2' : ''}>
                  ...
                  {field.type === 'textarea' ? (
                    <textarea
                      ...
                      className="w-full px-3 py-2 md:px-4 md:py-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 outline-none transition text-sm md:text-base"
                    />
                  ) : field.type === 'select' ? (
                    <select
                      name={field.name}
                      ...
                      value={formData[field.name] || ''}
                      onChange={handleChange}
                      className="w-full px-3 py-2 md:px-4 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition bg-white text-sm md:text-base"
                    >
            ...
            <div className="pt-2 md:pt-4">
              <button
                ...
                className={`w-full font-semibold py-3 md:py-4 rounded-lg transition-all shadow-md text-sm md:text-base ${status.loading ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
              >
          ...
          {/* Succès */}
          {status.submitted && (
            <div className="mt-6 bg-green-50 border border-green-200 rounded-xl p-4 md:p-5 text-center">
              <p className="text-green-800 font-medium mb-3 text-sm md:text-base">
                 Réservation enregistrée avec succès !<br />
                Cliquez ci-dessous pour finaliser sur WhatsApp :
              </p>
              {waUrl && (
                <a href={waUrl} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-3 bg-[#25D366] text-white font-semibold rounded-lg hover:bg-[#128C7E] transition shadow-md text-sm md:text-base">
              ...
              <p className="text-xs text-gray-500 mt-3">Le lien ouvre directement l'app WhatsApp ou WhatsApp Web.</p>
            </div>
          )}

          {/* Erreur */}
          {status.error && (
            <div className="mt-6 bg-red-50 text-red-800 p-4 rounded-lg text-center font-medium border border-red-200 text-sm md:text-base">
              {status.error}
            </div>
          )}
        </div>
